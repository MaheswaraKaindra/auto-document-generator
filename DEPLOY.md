...
- **HTTPS/domain** — Caddy SUDAH disediakan (`Caddyfile` + profile `proxy` di
  `docker-compose.yml`, TLS otomatis). "Colok"-nya = arahkan DNS domain ke server +
  jalankan `SITE_ADDRESS=domain docker compose --profile proxy up -d`. Lihat
  "HTTPS + domain (produksi)" di atas.
