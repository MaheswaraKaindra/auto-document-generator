...
119 test, **selalu mock** pemanggilan LLM (Claude), Mermaid.ink, dan GitHub — supaya test tidak bergantung pada koneksi internet, API key, atau kuota, dan tidak pernah mengeluarkan biaya API secara tidak sengaja.
...
**`parser_service.py` baru ter-cover di bagian deteksi endpoint** (7 test, sejak 2026-07-16 — bagian itu yang paling baru diubah dan paling mahal kalau salah). Ekstraksi class/function, heuristik `type`, dan jalur TS/JS masih tanpa test — lihat Keterbatasan.
...
- **`parser_service.py` baru ter-cover sebagian** (Peran 1). `tests/test_parser_endpoints.py` (7 test, 2026-07-16) mengunci deteksi endpoint Python — bagian yang paling baru diubah dan paling mahal kalau salah. Yang masih kosong: ekstraksi class/function, heuristik `type`, dan seluruh jalur TS/JS. `ingestion_service.py` juga masih tanpa test; ingestion sendiri sudah sebagian ter-cover lewat `tests/test_github_provider.py` (jalur tarball, GitHub di-mock lewat tarball sintetis di memori).
