...
- **Validasi menyentuh 10 repo, tapi masih cuma 3 yang pernah benar-benar jadi dokumen** (flask, fastapi, realworld). Ditambah 2026-07-15: `saleor-django` (Django e-commerce), `medusa-monorepo` (monorepo TS), `esteler-flask` (aplikasi Flask milik anggota tim — satu-satunya kode yang tidak ditulis untuk dipamerkan). Tahap 1 jalan ke semuanya; **Tahap 2 ke aplikasi bisnis masih belum pernah berhasil** — saleor ditolak guard context window (1,2 juta token), medusa juga akan ditolak (Contract A 3,4 MB). **Butir ini sebagian sudah terlampaui** — esteler-flask jadi dokumen pada 2026-07-16 (aplikasi bisnis Python nyata pertama, lihat Riwayat), jadi hitungannya kini **4**. `spring-petclinic` **sekarang terparse penuh** (Java didukung sejak 2026-07-16: 17 endpoint, 25 class) tapi **belum pernah jadi dokumen** — Tahap 2-nya belum dijalankan, dan Contract A-nya cuma 21 KB jadi murah (~$0,18). Yang masih benar-benar belum tersentuh: **Java sama sekali belum pernah melewati LLM**, jadi pertanyaan lama "kalau coverage rendah, apakah LLM mengarang dari nama file?" masih terbuka untuk Java — cuma sekarang dengan coverage 33%, bukan 0%.
