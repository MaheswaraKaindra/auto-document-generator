...
| `PUT` | `/templates/{id}/mappings/{doc_type}` | **Simpan peta bab hasil tinjauan manusia** → generate ulang template Jinja. Body: `{bindings: [...]}` PARALEL dengan peta tersimpan (cuma binding — struktur bab hasil pengukuran, bukan pendapat). **404** template/doc_type tak ada; **422** jumlah tak cocok / binding tak dikenal / satu isi dipakai dua bab |
