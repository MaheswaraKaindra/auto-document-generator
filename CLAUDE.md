...
- **16 placeholder `(diisi manual)` diserahkan mentah ke pengguna — ini masalah UX, bukan masalah generasi** (ditemukan 2026-07-15 saat membandingkan dengan dokumen acuan). Isinya: No. Solution Design, RFC #, Versi, Document Classification, baris-baris approval, dan checklist security. Semuanya hal yang **manusia tahu tapi kode tidak akan pernah tahu** — jadi memang benar tidak digenerate. Masalahnya bukan di situ: produk menyerahkan `.docx` berisi 16 lubang, lalu pengguna harus berburu satu-satu di Word. Yang seharusnya: **tanyakan di form sebelum generate**, lalu dokumennya keluar utuh. Perubahannya kecil (tambah field di `frontend/src/App.jsx` + `GenerateDocumentRequest` + template), tapi lompatan kesan profesionalnya besar: dari "draf berlubang" jadi "siap kirim". Ini kandidat perbaikan produk dengan rasio dampak-per-usaha tertinggi yang diketahui saat ini.
