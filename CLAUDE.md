...
138 test, **selalu mock** pemanggilan LLM (Claude), Mermaid.ink, dan GitHub — supaya test tidak bergantung pada koneksi internet, API key, atau kuota, dan tidak pernah mengeluarkan biaya API secara tidak sengaja.
...
| `tests/test_parser_endpoints.py` | Deteksi endpoint Python: Flask (`@bp.route`) & FastAPI (Peran 1) |
| `tests/test_parser_java.py` | Parser Java: endpoint Spring MVC (termasuk prefix class), interface, Javadoc, import (Peran 1) |

**`parser_service.py` ter-cover di bagian deteksi endpoint Python (7 test) dan seluruh jalur Java (19 test)** — dua bagian yang paling baru diubah dan paling mahal kalau salah. Ekstraksi class/function Python, heuristik `type`, dan jalur TS/JS masih tanpa test — lihat Keterbatasan.
...
- **Path Blueprint Flask tercatat tanpa `url_prefix`-nya** (diketahui 2026-07-16, belum diperbaiki). Yang diambil adalah path yang ditulis di decorator (`/login`), sementara blueprint biasanya didaftarkan di file lain (`app.register_blueprint(auth_bp, url_prefix="/auth")`) sehingga URL sebenarnya `/auth/login`. Gejalanya terlihat di esteler-app: `routes/admin.py` dan `routes/customer.py` sama-sama melaporkan `GET /`. Menyelesaikannya butuh analisis lintas-file (cari `register_blueprint`, cocokkan nama variabelnya). Dampaknya sedang: nama fitur dan use case tetap benar, tapi tabel endpoint di dokumen kurang tepat.
