...
Perilaku ini pernah diverifikasi lewat pemanggilan API sungguhan, **tapi dengan urutan prompt yang lama** (`target_doc_type` di depan). Sejak 2026-07-15 urutannya dibalik supaya Contract A bisa di-cache, jadi `target_doc_type` sekarang muncul **sesudah** Contract A. Secara teori ini justru lebih baik (instruksi tugas paling dekat dengan titik generation, dan system prompt tetap menjelaskan aturannya), tapi **belum diuji ulang** — kalau suatu saat SDD dan UAT terasa mirip lagi, curigai ini lebih dulu.
