...
tests/                       # pytest (117 test) — lihat bagian Testing
...
117 test, **selalu mock** pemanggilan LLM (Claude), Mermaid.ink, dan GitHub — supaya test tidak bergantung pada koneksi internet, API key, atau kuota, dan tidak pernah mengeluarkan biaya API secara tidak sengaja.
...
- **Contract A monorepo masih terlalu besar untuk didokumentasikan** (diukur 2026-07-16). Sesudah `workspaces` dibaca, medusa turun 9.459 → 7.355 file, tapi Contract A-nya masih **2.831 KB** — jauh di atas context window 1M token, jadi medusa **tetap tidak bisa dibuatkan dokumen** (ditolak `_guard_context_window`). Menyaring lebih jauh tidak akan menolong: 7.297 file yang tersisa itu **memang kode produk**. Ini masalah yang berbeda dan lebih dalam: **produk ini mengirim SELURUH Contract A dalam satu panggilan**, dan untuk monorepo sungguhan itu tidak akan pernah muat. Arah yang mungkin: pecah per-workspace-package jadi beberapa panggilan lalu gabungkan, atau ringkas dulu sebelum dikirim. Ini penghalang nyata untuk melayani aplikasi bisnis besar.
